...
const BACKEND_URL =
  ...
  "https://chatapp-backend-chyk.onrender.com";
...
const releaseNotesArg =
  notesIndex !== -1 ? process.argv.slice(notesIndex + 1).join(" ") : undefined;

console.log(
  "Building Android production APK via EAS (this can take several minutes)...",
);
...
const buildNumber =
  Number(build.appBuildVersion || build.metadata?.appBuildVersion) || 1;
const releaseNotes =
  releaseNotesArg || `FlowChat v${version} (Build #${buildNumber})`;
...
console.log(
  "The web app's download button and QR code now point to this release automatically.",
);
