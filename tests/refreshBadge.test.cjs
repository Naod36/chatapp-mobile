...
test("refresh badge uses the existing indicator and restores the normal states", () => {
  ...
  const { default: Badge } = runner.load("src/components/common/SyncBadge.js", {
    "react-native": {
      ...
      Animated: { View: "AnimatedView", Value: function(value) { this.value = value; },
        timing: () => ({}), sequence: () => ({}), loop: () => ({ start() {}, stop() {} }) },
    },
  });
  for (const [syncState, expected] of [["refreshing", "Refreshing..."], ["connecting", "Connecting..."], ["updating", "Updating..."]]) {
    const tree = runner.render(() => Badge({ syncState, theme: {} }));
    assert.ok(nodes(tree).some((node) => node.type === "Text" && node.props.children.includes(expected)));
  }
  assert.equal(runner.render(() => Badge({ syncState: "ready", theme: {} })), null);
  runner.unmount();
});