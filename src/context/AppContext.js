...
export function AppProvider({ children }) {
    ...
    const logout = useCallback(async () => {
        try { websocketService.closeAll(); } catch (e) {}
    ...
    useEffect(() => {
        AsyncStorage.getItem(THEME_KEY)
            .then(saved => { if (saved && THEMES[saved]) setThemeKey(saved); })
            .catch(() => {});
    }, []);
    ...
    // ─── Conversations ───────────────────────────────────────────────────────
    const loadConversations = useCallback(async (showUpdating = true) => {
    ...
    useEffect(() => {
        ...
        const handler = (data) => {
            const currentUserId = String(userRef.current?.userId || userRef.current?.user_id || "");
            ...
            if (event === "new_message") {
                ...
                else if (msg.message_type === "voice" || msg.message_type === "audio") preview = "🎤 Voice message";
                else if (msg.message_type === "file") preview = `📎 ${msg.file_name || "File"}`;
                else if (msg.message_type === "video") preview = "🎥 Video";

                setConversations(prev => {
                    const idx = prev.findIndex(c => String(c.id || c.conversation_id) === convIdStr);
                    if (idx === -1) {
                    ...
                    const conv = {
                        ...
                        unread_count: String(msg.sender_id) !== currentUserId
            ...
            // ── Presence ─────────────────────────────────────────────────────
                ...
                setConversations(prev =>
                    prev.map(c => {
                        const otherId = String(c.other_participant?.user_id || c.other_participant?.id || "");
                        if (otherId && otherId === uid) {
                            return { ...c, status, other_participant: { ...c.other_participant, status } };
                        }
                        return c;
                    })
                );
            ...
            // ── Read / delivered ─────────────────────────────────────────────
                ...
                if (!convIdStr) return;
                setConversations(prev =>
                    prev.map(c => String(c.id) === convIdStr ? { ...c, unread_count: 0 } : c)
                );
            }
        };
    ...
    const markConversationRead = useCallback((convId) => {
        setConversations(prev =>
            prev.map(c => String(c.id) === String(convId) ? { ...c, unread_count: 0 } : c)
        );
    }, []);
    ...
    const value = {
        ...
        themeKey, theme: t, changeTheme,
        conversations, setConversations, syncState, loadConversations, markConversationRead,
        presenceMap, getPresence,
        typingMap,
    };
