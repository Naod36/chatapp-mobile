...
export default function NewGroupScreen({ navigation }) {
    ...
    return (
        <View style={[styles.container, { backgroundColor: t.bg, paddingTop: insets.top }]}>
            {/* Header */}
            <View style={[styles.header, { borderBottomColor: t.borderColor, backgroundColor: t.headerBg }]}>
                ...
                <TouchableOpacity onPress={handleCreate} disabled={creating} style={styles.createBtn}>
                    {creating
                        ...
                        : <Text style={[styles.createLabel, { color: t.accent }]}>Create</Text>
                    }
                </TouchableOpacity>
            </View>

            {/* Group name input */}
            <View style={[styles.groupNameWrap, { borderBottomColor: t.borderColor }]}>
                <TextInput
                    value={groupName}
                    onChangeText={setGroupName}
                    placeholder="Group name (required)"
                    placeholderTextColor={t.textMuted}
                    style={[styles.groupNameInput, { color: t.text, backgroundColor: t.inputBg, borderColor: t.borderColor }]}
                    maxLength={80}
                    returnKeyType="done"
                />
            </View>
                ...
                <View style={[styles.chips, { borderBottomColor: t.borderColor }]}>
                    {selected.map(u => (
                        <TouchableOpacity
                            ...
                            style={[styles.chip, { backgroundColor: t.accent + "22", borderColor: t.accent }]}
            ...
            {/* Results */}
            {searching ? (
                <ActivityIndicator style={{ marginTop: 30 }} size="large" color={t.accent} />
            ) : (
                <FlatList
                    data={results}
                        ...
                        return (
                            ...
                            >
                                ...
                                <View style={styles.info}>
                                    ...
                                    <Text style={[styles.handle, { color: t.textMuted }]}>@{item.username}</Text>
                                </View>
                                <View style={[
                                    styles.checkCircle,
                                    { borderColor: sel ? t.accent : t.borderColor, backgroundColor: sel ? t.accent : "transparent" },
                                ]}>
                    ...
                    ListEmptyComponent={
                        query.trim() && !searching ? (
                            <View style={styles.empty}>
                                <Text style={[styles.emptyText, { color: t.textMuted }]}>No users found</Text>
                            </View>
                        ) : null
                    }
