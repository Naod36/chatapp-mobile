...
export function useMessages(
      ...
      .then(([msgsData, pinsData]) => {
        ...
        setMessages((previous) => {
          ...
          sortedMsgs.forEach((message) =>
            byId.set(String(message.id || message.message_id), message),
          );
        ...
        if (Array.isArray(pinsData) && pinsData.length > 0) {
          ...
          const fallback = sortedMsgs.find(
            (m) =>
              String(m.id || m.message_id) === String(conversationPinnedId),
          );
          if (fallback) {
  ...
  useEffect(() => {
    ...
    const isCurrentConv = (data) => {
      ...
      return (
        String(id || "").toLowerCase() === String(convId || "").toLowerCase()
      );
    };
    ...
    const handler = (data) => {
      ...
      if (event === "new_message" && isCurrentConv(data)) {
        ...
        setMessages((prev) => {
          if (
            msgId &&
            prev.some((m) => String(m.id || m.message_id) === String(msgId))
          ) {
            return prev.map((message) =>
              String(message.id || message.message_id) === String(msgId)
                ? {
                    ...message,
                    ...msg,
                    id: msgId,
                    ...
                    status:
                      ["read", "delivered"].find(
                        (status) =>
                          status === message.status || status === msg.status,
                      ) ||
                      msg.status ||
                      message.status ||
                      "sent",
        ...
        setMessages((prev) =>
          prev.map((m) =>
            !["read", "failed", "sending"].includes(m.status)
              ? { ...m, status: "delivered" }
              : m,
          ),
        );
        ...
        setMessages((prev) =>
          prev.filter((m) => String(m.id || m.message_id) !== deletedId),
        );
        setPinnedMessages((prev) =>
          prev.filter((p) => String(p.message_id || p.id) !== deletedId),
        );
        ...
        setPinnedMessages((prev) =>
          prev.filter((p) => String(p.message_id || p.id) !== unpinnedId),
        );
        ...
        setMessages((prev) =>
          prev.map((m) => {
            ...
            return {
              ...m,
              reactions: Object.keys(currentRx).length > 0 ? currentRx : null,
            };
          }),
        );
      } else if (
        (event === "typing" || event === "typing_status") &&
        isCurrentConv(data)
      ) {
        ...
        if (
          ...
          interactionRef.current() ||
          !senderId ||
          getBlockPolicy(senderId).hideIdentity
        )
          return;
  ...
  const sendMessage = useCallback(
    async (
      ...
      try {
        const confirmed = await conversationService.sendMessage(
          convId,
          contentStr,
          messageType,
          replyToId,
        ...
        if (!confirmed?.id && !confirmed?.message_id)
          throw new Error("The server did not confirm this message.");
        setMessages((prev) => {
          const confirmedId = String(confirmed.id || confirmed.message_id);
          const optimistic =
            prev.find((message) => message.id === tempId) || tempMsg;
          const echo = prev.find(
            (message) =>
              String(message.id || message.message_id) === confirmedId,
          );
          return prev
            .filter(
              (message) =>
        ...
        setMessages((prev) =>
          prev.map((message) =>
            message.id === tempId
              ? { ...message, status: "failed", send_error: err.message }
              : message,
          ),
        );
        throw err;
  ...
  const pinMessage = useCallback(
    async (msgId, scope = "shared", notify = true) => {
      assertInteractionAllowed();
      ...
      setPinnedMessages((prev) => [
        pinItem,
        ...prev.filter((p) => String(p.message_id || p.id) !== String(msgId)),
      ]);
  ...
  const deleteMessage = useCallback(async (msgId) => {
    ...
    try {
      ...
      setMessages((prev) =>
        prev.filter((m) => String(m.id || m.message_id) !== String(msgId)),
      );
      setPinnedMessages((prev) =>
        prev.filter((p) => String(p.message_id || p.id) !== String(msgId)),
      );
  ...
  return {
    messages: messages.map((message) =>
      redactMessage(
        message,
        (userId) => !blockStateReady || isBlockedBy(userId),
      ),
    ),
    loading,
