...
function ConversationItem({ conversation: c, onPress, isTyping }) {
    ...
    const isOnline = !isGroup && !isSaved && (
    ...
    const name = isSaved
        ? "Saved Messages"
        : (c.display_name || c.title || (otherUser?.display_name) || (otherUser?.username) || "Chat");
    ...
    const preview = isTyping
        ? "typing..."
        : (rawPreview || (isGroup ? `${c.participants?.length || 0} members` : "No messages yet"));


    const isOwnLastMsg = c.last_message && String(c.last_message.sender_id) === String(user?.userId);
            ...
            <View style={styles.body}>
                <View style={styles.topRow}>
                    ...
                    <Text
                        style={[
                            styles.time,
                            { color: hasUnread ? t.accent : t.textMuted },
                        ]}
                    >
