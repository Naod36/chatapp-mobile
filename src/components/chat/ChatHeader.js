...
export default function ChatHeader({ conversation, typingUser, onBack }) {
    ...
    const { theme: t, getPresence, typingMap } = useApp();

    const convIdStr = String(conversation?.id || conversation?.conversation_id || "");
    const isGroup = conversation?.type === "group";
    const isSaved = !conversation?.other_participant && conversation?.type === "direct";
            ...
            <View style={styles.titleArea}>
                ...
                {subtitle ? (
                    <Text
                        style={[
                            styles.subtitleText,
                            { color: typingUser ? t.accent : (isOnline ? "#22c55e" : t.textMuted) },
                        ]}
                        numberOfLines={1}
                    >
                        {subtitle}
