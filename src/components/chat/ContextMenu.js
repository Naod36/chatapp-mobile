 ...
 */
export default function ContextMenu({ visible, message, isOwn, isPinned, theme: t, onReact, onReply, onCopy, onEdit, onPin, onDelete, onClose }) {
            ...
            <TouchableWithoutFeedback onPress={onClose}>
                <View style={styles.backdrop}>
                    <TouchableWithoutFeedback>
                        <View style={[styles.panel, { backgroundColor: t.cardBg, borderColor: t.borderColor }]}>
                            {/* Emoji Quick Reactions Bar */}
                            <View style={[styles.emojiBar, { borderBottomColor: t.borderColor }]}>
                            ...
                            {message.content ? (
                                <Text style={[styles.preview, { color: t.textMuted }]} numberOfLines={2}>
                                    {message.content}
                                </Text>
                            ) : null}
                            ...
                            {actions.map((action, i) => (
                                ...
                                >
                                    <Text style={[styles.actionText, { color: action.danger ? "#ef4444" : t.text }]}>
                                        {action.label}
                                    </Text>
                                </TouchableOpacity>
                            ))}

                            <TouchableOpacity style={[styles.action, styles.cancelAction, { borderTopColor: t.borderColor }]} onPress={onClose}>
                                <Text style={[styles.actionText, { color: t.textMuted }]}>Cancel</Text>
                            </TouchableOpacity>
                        </View>
                    </TouchableWithoutFeedback>
                </View>
