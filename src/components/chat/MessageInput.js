...
export default function MessageInput({
    ...
    const handlePickImage = async () => {
        ...
        try {
            ...
            if (status !== 'granted') {
                Alert.alert("Permission Needed", "FlowChat needs access to your photo library to share photos and videos.");
                return;
            }
            ...
            if (!result.canceled && result.assets && result.assets.length > 0) {
                ...
                onSelectAttachment?.({
                    uri: asset.uri,
                    name: asset.fileName || asset.name || (isVideo ? `video_${Date.now()}.mp4` : `photo_${Date.now()}.jpg`),
    ...
    return (
        <View style={[styles.outerWrap, { backgroundColor: t.bg, borderTopColor: t.borderColor, paddingBottom: dynamicPaddingBottom }]}>
            ...
            {editingMessage && (
                <View style={[styles.replyBar, { backgroundColor: t.cardBg, borderColor: t.borderColor }]}>
                    <View style={styles.replyBarInner}>
                        <Text style={[styles.replyLabel, { color: t.accent }]} numberOfLines={1}>
                            ✏️ Editing message
                        </Text>
                        <Text style={[styles.replyContent, { color: t.textMuted }]} numberOfLines={1}>
            ...
            {attachment && (
                <View style={[styles.previewBar, { backgroundColor: t.cardBg, borderColor: t.borderColor }]}>
                    {attachment.mediaType === "image" ? (
                        <Image source={{ uri: attachment.uri }} style={styles.previewImage} />
                    ) : (
                        <View style={[styles.previewIconWrap, { backgroundColor: t.accent + "20" }]}>
                            <Text style={{ fontSize: 20 }}>
                                {attachment.mediaType === "video" ? "🎥" : attachment.mediaType === "voice" ? "🎙️" : "📎"}
                            </Text>
                        </View>
                    )}
                    <View style={styles.previewTextWrap}>
                        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                            <Text style={[styles.previewName, { color: t.text, flex: 1, marginRight: 8 }]} numberOfLines={1}>
                                {attachment.name}
                            </Text>
                        ...
                        {uploadProgress ? (
                            <View style={{ marginTop: 4 }}>
                                <View style={{ height: 4, borderRadius: 2, backgroundColor: t.isDark ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.1)", overflow: "hidden" }}>
                                    <View style={{ height: "100%", backgroundColor: t.accent, width: `${uploadProgress.percentage}%` }} />
                                </View>
                                <Text style={{ fontSize: 10.5, marginTop: 3, fontWeight: "600", color: t.textMuted }}>
                                    Uploading {uploadProgress.percentage}% ({uploadProgress.loadedFormatted} / {uploadProgress.totalFormatted})
                                </Text>
                            </View>
                        ) : (
                    ...
                    {!isUploading && (
                        <TouchableOpacity onPress={onClearAttachment} style={styles.clearBtn}>
                            <Text style={{ color: t.textMuted, fontSize: 16, fontWeight: "700" }}>✕</Text>
                        </TouchableOpacity>
                    )}
                </View>
            )}
            ...
            {isRecording ? (
                <View style={[styles.inputRow, { backgroundColor: t.bg }]}>
                    <TouchableOpacity onPress={cancelRecording} style={[styles.attachBtn, { backgroundColor: t.cardBg, borderColor: t.borderColor }]}>
                        <Text style={{ color: t.textMuted, fontWeight: "700", fontSize: 14 }}>✕</Text>
                    </TouchableOpacity>
                    ...
                    <View style={[styles.inputWrap, styles.recordingActiveWrap, { backgroundColor: t.cardBg, borderColor: t.accent }]}>
                        ...
                        <View style={styles.liveWaveform}>
                            {[40, 75, 35, 90, 60, 100, 45, 80, 50, 35, 70, 40].map((h, idx) => (
                                <View
                                    key={idx}
                                    style={[
                                        styles.liveBar,
                    ...
                    <TouchableOpacity onPress={stopAndSendRecording} style={[styles.sendBtn, { backgroundColor: t.accent }]}>
                        <SendIcon color="#fff" />
                    </TouchableOpacity>
                </View>
            ) : (
                <View style={[styles.inputRow, { backgroundColor: t.bg }]}>
                    ...
                    <TouchableOpacity
                        ...
                        style={[styles.attachBtn, { backgroundColor: t.cardBg, borderColor: t.borderColor }]}
                        activeOpacity={0.7}
                    >
                        ...
                        <TouchableWithoutFeedback>
                            <View style={[styles.modalSheet, { backgroundColor: t.cardBg, borderColor: t.borderColor, paddingBottom: Math.max(insets.bottom + 16, Platform.OS === "ios" ? 40 : 24) }]}>
                                ...
                                <TouchableOpacity style={[styles.optionRow, { borderBottomColor: t.borderColor }]} onPress={handlePickImage}>
                                    ...
                                    <View>
                                        <Text style={[styles.optionText, { color: t.text }]}>Photo or Video</Text>
                                        <Text style={[styles.optionSub, { color: t.textMuted }]}>Choose from your gallery</Text>
                                    </View>
                                </TouchableOpacity>
                                <TouchableOpacity style={styles.optionRow} onPress={handlePickDocument}>
                                    <Text style={{ fontSize: 22, marginRight: 12 }}>📄</Text>
                                    <View>
                                        <Text style={[styles.optionText, { color: t.text }]}>Document or File</Text>
                                        <Text style={[styles.optionSub, { color: t.textMuted }]}>Choose from device files</Text>
                                    </View>
                                </TouchableOpacity>
                            </View>
                        </TouchableWithoutFeedback>
