...
const SENDER_COLORS = ["#0284c7", "#818cf8", "#f43f5e", "#d97706", "#059669", "#7c3aed", "#db2777"];
...
function MessageBubble({ msg, isOwn, isGroup, theme: t, currentUserId, participants, otherParticipant, userProfile, onToggleReaction, onLongPress }) {
    ...
    return (
        ...
        >
            ...
            {!isOwn && isGroup && (
                <View style={[styles.groupAvatar, { backgroundColor: senderColor(msg.sender_id || msg.user_id) }]}>
                    <Text style={styles.groupAvatarText}>{(msg.sender_name || "?")[0].toUpperCase()}</Text>
                </View>
            )}
            ...
            <View style={[styles.bubbleWrap, { alignItems: isOwn ? "flex-end" : "flex-start" }]}>
                ...
                >
                    ...
                    {isGroup && !isOwn && msg.sender_name && (
                        <Text style={[styles.senderName, { color: senderColor(msg.sender_id || msg.user_id) }]}>
                            {msg.sender_name}
                        </Text>
                    )}
                    ...
                    {isFile && (
                        <View style={styles.mediaRow}>
                            ...
                            <Text style={[styles.mediaLabel, { color: textColor }]} numberOfLines={1}>
                                {msg.file_name || "File"}
                            </Text>
                        </View>
                    )}
                    ...
                    {isVideo && (
                        <View style={styles.mediaRow}>
                            <Text style={{ fontSize: 16 }}>🎥</Text>
                            <Text style={[styles.mediaLabel, { color: textColor }]}>Video</Text>
                        </View>
                    )}
                ...
                {msg.reactions && Object.keys(msg.reactions).length > 0 && (
                    <View style={[styles.reactionsRow, { justifyContent: isOwn ? "flex-end" : "flex-start" }]}>
                        {Object.entries(msg.reactions).map(([emoji, uids]) => {
                            ...
                            const reactUsers = uids.map(uid => {
                                ...
                                const part = (participants || []).find(p => String(p.user_id || p.id) === normUid);
                                if (part) {
                                    return {
                                ...
                                if (otherParticipant && String(otherParticipant.user_id || otherParticipant.id) === normUid) {
                                    return {
                                        avatar: otherParticipant.avatar_url,
                                        initial: (otherParticipant.display_name || otherParticipant.username || "?")[0].toUpperCase()
                                    };
                                }
                            ...
                            return (
                                    ...
                                    style={[
                                        styles.reactionPill,
                                        {
                                            backgroundColor: hasReacted
                                                ? (t.isDark ? "rgba(63,224,197,0.18)" : "rgba(2,132,199,0.12)")
                                ...
                                >
                                            ...
                                            <View
                                                ...
                                                style={[
                                                    styles.rxAvatar,
                                                    {
                                                        ...
                                                        backgroundColor: u.avatar ? "transparent" : "#4A3FE0",
                                                    },
                                                ]}
                                            >
                                                {u.avatar ? (
                                    ...
                                    {uids.length > 1 && (
                                        <Text style={[styles.reactionCount, { color: hasReacted ? "#3FE0C5" : t.textMuted }]}>
                                            {uids.length}
                                        </Text>
                                    )}
                                </TouchableOpacity>
                ...
                <View style={styles.footer}>
                    <Text style={[styles.time, { color: t.textMuted }]}>
                        {formatTime(msg.created_at || msg.timestamp)} {msg.is_edited ? "(edited)" : ""}
                    </Text>
                    <MessageStatusIcon status={msg.status} isOwn={isOwn} isDark={t.isDark} />
                </View>
            </View>
        </TouchableOpacity>
    );
